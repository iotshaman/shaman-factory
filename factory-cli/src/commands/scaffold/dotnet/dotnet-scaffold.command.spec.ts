...
describe('Scaffold DotNet Environment Command', () => {
  ...
  it('run should throw if invalid template name provided', (done) => {
    let mockSolution = new MockSolution();
    mockSolution.projects = [new MockDotnetProject()];
    mockSolution.projects[0].template = '';
    let subject = new DotnetScaffoldCommand(mockSolution, './');
    subject.run(mockSolution.projects[0])
      .then(_ => { throw new Error("Expected rejected promise, but promise completed.") })
      .catch((ex: Error) => {
        expect(ex.message).to.equal("Invalid template name configuration (project=Test).");
        done();
      });
  });
  ...
  it('run should not add solution file if solution file already exists', (done) => {
    ...
    subject.run(mockSolution.projects[0]).then(_ => {
