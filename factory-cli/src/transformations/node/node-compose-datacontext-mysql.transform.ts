...
export class NodeComposeMysqlDataContextTransformation implements ITransformation {

  get name(): string { return "compose:datacontext-mysql"; }
  ...
  transform = (transformation: ProjectTransformation, solution: Solution, solutionFolderPath: string): Promise<void> => {
    ...
    return this.sourceService.createAppConfigFromSampleConfig(solutionFolderPath, project)
      ...
      .then(_ => this.sourceService.addDataContextComposition(solutionFolderPath, project, databaseProject.name, contextName, "mysqlConfig"));
  }
